...
It can be run manually and is also scheduled weekly. The weekly gate is implemented inside `build_solar_size_split_charts.py` through `data/metadata/solar_size_split_chart_state.json`, so a scheduled run in a week that has already been rendered exits without changing outputs. Manual runs use `--force` and always produce a render.

The workflow is intentionally separate from the daily grid-data refresh so chart rendering does not add plotting dependencies or runtime to the daily source checks.
