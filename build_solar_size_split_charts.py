...
SIZE_JSON = Path("data/distributed_generation/model/solar_size_buckets_current.json")
...
def fit_mid_rate(meta: dict, monthly_rows: list[dict[str, str]]) -> float:
...
def history_points() -> list[dict[str, object]]:
    ...
    for month in sorted(grouped):
        if not all(g in grouped[month] for g in GROUPS):
            continue
...
def future_points(observed: list[dict[str, object]]) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    mid_rate = fit_mid_rate(meta, rows)
    ...
    return future, {"mid_20pct_growth_rate_per_year": mid_rate, "utility_future_policy": "hold current observed utility-scale bucket flat; project-timed utility additions are not extrapolated generically"}


def save_plot_data(points: list[dict[str, object]]) -> None:
    fields = sorted({key for point in points for key in point.keys()})
...
def render(points: list[dict[str, object]], metric: str, out_path: Path) -> None:
    months = [datetime.strptime(str(p["month"]), "%Y-%m-%d").date() for p in points]
    x = np.arange(len(points))
    small = np.array([float(p[f"small_{metric}"]) for p in points])
    larger = np.array([float(p[f"larger_{metric}"]) for p in points])
    utility = np.array([float(p[f"utility_{metric}"]) for p in points])
    ...
    if model_idx:
        ...
        ax.fill_between(mx, low, high, alpha=0.12, label="10–30% small-system saturation range")
        ax.plot(mx, low, linestyle="--", linewidth=1.2, label="10% saturation")
        ax.plot(mx, mid, linewidth=2.2, label="20% saturation")
        ax.plot(mx, high, linestyle="--", linewidth=1.2, label="30% saturation")
