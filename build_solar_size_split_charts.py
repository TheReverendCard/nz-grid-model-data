...
def future_points(observed: list[dict[str, object]]) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    # Utility-scale solar is deliberately not extrapolated by a generic growth rate.
    # It stays at the latest observed bucket until a project-timed utility model is added.
...
def render(points: list[dict[str, object]], metric: str, out_path: Path) -> None:
    ...
    # Connecting cumulative boundaries make gradual distributed growth and step-like
    # utility additions easy to see as the observed monthly history accumulates.
