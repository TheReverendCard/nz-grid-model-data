...
WAITAKI_STORAGE_COLUMNS = ["TKA_energy_equivalent_gwh", "PKI_energy_equivalent_gwh", "OHA_energy_equivalent_gwh"]
...
def load_waitaki_storage() -> dict[str, float]:
    ...
    with STORAGE_ENERGY.open(encoding="utf-8", newline="") as handle:
        for row in csv.DictReader(handle):
            ...
            if len(vals) == len(WAITAKI_STORAGE_COLUMNS):
                rows[d] = sum(vals)
    return rows
...
def solar_month_weights() -> dict[int, float]:
    ...
    return {int(m): float(v) for m, v in seasonality["solar_method"]["monthly_index_vs_annual_mean_daily"].items()}
...
def dispatch_policy(policy: str, annual_gwh: float, dates: list[str], solar: dict[str, float],
                    storage: dict[str, float], waitaki_gen: dict[str, float], thermal_gen: dict[str, float],
                    empirical_ceiling_gwh: float) -> tuple[list[dict[str, object]], dict[str, float]]:
    ...
    curtailed = 0.0
    storage_limited = 0.0
    foregone_pre_winter_thermal_savings = 0.0
    rows = []
    ...
    for d in dates:
        ...
        # Policy choice is explicit. Fuel-minimizing always displaces thermal first.
        # Winter-security gives hydro conservation priority before 1 May, then reverts
        # to thermal-first dispatch once winter begins.
        hydro_first = policy == "winter_security" and d < WINTER_START

        solar_to_thermal = 0.0
        hydro_stored = 0.0
        storage_limited_mwh = 0.0

        if hydro_first:
            hydro_candidate = min(solar_remaining, observed_waitaki)
            headroom_mwh = max(0.0, (empirical_ceiling_gwh - observed_storage - delta_storage_gwh) * 1000.0)
            ...
            # Relative to the fuel-minimizing rule, this is the amount of pre-winter
            # thermal displacement deliberately forgone in order to hold more water.
            max_possible_thermal_first = min(extra_solar, observed_thermal)
            foregone_pre_winter_thermal_savings += max(0.0, max_possible_thermal_first - solar_to_thermal)
        else:
            solar_to_thermal = min(solar_remaining, observed_thermal)
            direct_thermal_displaced += solar_to_thermal
            solar_remaining -= solar_to_thermal
    ...
    result = {
        ...
        "foregone_pre_winter_direct_thermal_savings_gwh": round(foregone_pre_winter_thermal_savings / 1000.0, 3),
...
def main() -> None:
    ...
    daily_rows = []
    policy_results: dict[str, dict[str, dict[str, float]]] = {}

    for policy in POLICIES:
        policy_results[policy] = {}
        for annual_gwh in SOLAR_SCENARIOS_GWH:
            solar = scenario_daily_solar(annual_gwh, dates, month_index)
            rows, result = dispatch_policy(policy, annual_gwh, dates, solar, storage, waitaki_gen, thermal_gen, empirical_ceiling_gwh)
    ...
    summary = {
        "status": "policy_comparison_delta_dispatch_diagnostic",
        "purpose": "Separate hydrological opportunity from dispatch-policy choice when assessing whether incremental solar can preserve Waitaki water for winter.",
        "method": {
            ...
            "policies": {
                ...
                "winter_security": "Before 1 May solar conserves Waitaki hydro first, subject to storage headroom; from 1 May onward solar displaces thermal first and conserved hydro can also be released against remaining thermal generation."
            },
        ...
        "important_limitations": [
            "This remains a counterfactual delta model, not a full physical Waitaki water-routing model.",
            "The policy comparison is intentional: zero winter-entry storage under fuel-minimizing dispatch is a dispatch-rule outcome, not a hydrological conclusion.",
            "Winter-security does not increase thermal above the observed 2024 baseline; it reports thermal savings forgone relative to thermal-first solar dispatch.",
            "Observed 2024 storage already embodies natural inflows, tributaries, diversions, spill, environmental releases and operator behaviour.",
